...
| 3 | LL-29 … LL-42 | `room/lead-2026-09-16-longlist-feasibility-batch-3.md` | `room/steward-2026-09-16-longlist-feasibility-batch-3-answers.md` | **finalised**: 6 FEASIBLE, 7 FEASIBLE-WITH-CAVEAT, 1 NOT FEASIBLE (LL-34, deleted); **LL-32 also deleted by the lead** as subsumed by LL-36 and exhausted by the steward's own audit |
...
that three primitive levels break at the June 2026 boundary; batch 3's are at `(g)` (twelve more),
with three corrections made in place — the hours/minutes trap, the June boundary list (`multitasking`
is a fourth breaking metric and `task_success` has no June counterpart), and the verification of the
**OpenAI Signals bundle**, which carries the first cross-provider task key found so far (149 of its
165 IWA ids match the June wave's `onet` level-2 nodes).

Caveats that changed a design rather than confirming it, by batch. **Batch 1:** the `request` ladder
has almost no cross-wave node correspondence (LL-06 moved to `onet_task`), the RPS occupation index
is published at SOC minor group (LL-03 runs at N = 90), and a country's published task mix covers
only about 30% of its conversations (LL-06's suppression control changed). **Batch 2:** LL-16's
`steward?` flag is settled with numbers, LL-19's residual is the size of its own measurement error,
LL-26's published task-value level does not reproduce under any of five wage constructions, and
LL-28's June wave is off its own autonomy axis. **Batch 3:** LL-40's internal half is our own atlas
restated and survives only on its external leg, LL-41's speedup is near-invariant to the threshold
so the published sensitivity lives elsewhere, LL-42's premise was wrong in our favour (all three
quantities co-exist in one wave at N = 116), and LL-39 came back with a three-tier list of which
metrics cross the June boundary, which is now that entry's spine.

## Counts

| | |
|---|---|
| drafted | 42 (LL-01 … LL-42) |
| deleted | 3 — LL-25 and LL-32 by the lead as subsumed, LL-34 by the steward as NOT FEASIBLE; reasons in the deletions table |
| surviving | **39**, every one of them carrying a steward feasibility line verbatim |

The gate asks for thirty or more candidates that survive all three gates; thirty-nine do. Every
surviving entry has been through the gap check against the corpus and against named external work,
carries a cut the steward has confirmed at the grain the question needs, and states a contribution
for each of the three outcomes. No entry is scored here; scoring is Step 2.
...
**Steward feasibility line** (verbatim; `room/steward-2026-09-16-longlist-feasibility-batch-3-answers.md`).
...
**Biggest risk.** Construct. Multitasking is a classifier judgement with no validation statistic, and
it may simply track conversation length, which is published nowhere (`ATLAS §Cuts 27`). If it bites,
the multi-task share is a length proxy and every correlation with it is mechanical; the post says so
and reports the relationship with the time primitives, the only length-adjacent measures available.
One new fact sharpens the stake rather than the risk: `multitasking_pct` is one of the metrics that
**breaks** at the June boundary (+12.72 pp, `ATLAS §Components`), so this post is confined to the two
2026 long waves and must not reach for June.
...
wave with no geography, and the residual label differs not just between waves but *within* the
February wave — `none` at global, both labels in the intersections. If it bites, a hard-coded
residual silently drops or double-counts part of the mass; the loader therefore reads both labels at
both grains by rule, and the post reports the 87.92 / 86.96 covered mass before any comparison, and
treats the two waves as a replication rather than a trend.
...
**Biggest risk.** Design, now that the publication risk is gone: with 114 countries and 536
subregions carrying all 32 metrics in both months, the binding problem is that artifact shares are
compositional and two-decimal rounded, so a place with a distinctive mix on one small category can
be noise. If it bites, the outliers are small categories in small units; the post therefore applies
the steward's `pct` ≥ 0.5 floor wherever a ratio is formed, requires both months, and reports the
April–May agreement for every claim — the within-wave replication LL-22 measures in general.
...
**Biggest risk.** Design, and the steward's audit has both simplified and sharpened it. There is one
substantive residual node, `none`, not two — so the decomposition is cleaner than the entry assumed —
but on the API it runs 10.21 → 11.04 → **15.19**, a rise of five points across the three waves,
which is the same order as the automation fall the fifth report reports. If it bites, the published
API automation movement and the residual movement are not separable without knowing what `none`
means on a prompt–response record, which no document states; the post therefore reports the two
series together, refuses the June wave, and puts the interpretive question to Anthropic rather than
answering it.
...
country — so a positive result has at least three readings, on a panel of 113 countries. If it
bites, the coefficient is an income coefficient; the pre-registration states the income and
composition controls in advance and reports the result with and without them, and the post's claim
is about places, never about users.
...
**Biggest risk.** Design, not data: the reconstruction reproduces the published facet to 0.02 pp, so
the risk moves entirely onto the **base**. The same series reads 39.03 → 36.02 → 32.23 on the
classified base and 35.86 → … on the all-conversation base, and the corpus quotes both without
always saying which. If it bites, a reader compares our number with a published number computed on
the other base and concludes we are wrong; the post therefore names the base in the sentence that
carries every number, reports both series in the table, and states the reproduction check first.
**This candidate also absorbs LL-32** (deleted): the allocation rule enters here as a pre-registered
robustness cut, with the steward's measured bound — one holder for all but 4.05% / 5.57% / 4.50% of
named usage mass, SOC-15 moving ≤0.17 pp and the 22-group ranking by at most one position.
...
false-positive rates but not predictive validity. The steward's admission rules bound the series to
five windows on names (1,868 nodes) or three on counts (2,282 nodes), with June only as a terminal
cross-section. If it bites, the honest framing is "what a trigger could look like and how often it
would fire spuriously", not "here is a working early-warning signal" — and the post must refuse the
stronger claim in its title as well as its text.
...
**Biggest risk.** Design — scope, and the steward has just widened it by naming two more dials. Six
observable quantities in one post is the failure lesson 5 records. If it bites, the post sprawls; the
pre-registration therefore fixes **two** confirmatory dials (ψ and m, the two the paper itself
anchors on) and treats `human_only_ability`, `task_success` and a as exploratory bounds in an audit
table, with the deliverable stated in advance as the table plus two bounded dials, not six estimates.
...
attributed to any one of them, and the June wave publishes no counts, so nothing can be tested. The
steward's three tiers make the post possible without solving that: the comparable set moves by less
than 1.1 while five metrics move by 0.6 to 12.7, which is the contrast the post rests on. If it
bites, the tiers are a ranking rather than an attribution; the post reports magnitudes with no
tests, requires April and May to agree in sign and size, and does not name the classifier, the
O*NET vintage or the window as the cause.
...
**Economic Index cut (proposed, amended on the steward's ruling).** The internal accounting is
**demoted to context**, because the steward states plainly that it is our own atlas restated: counts
against `_pct` sums, the floor of exactly 15, the denominators 964,494 / 999,875 / 1,000,000, the
`none`/`not_classified` mass by grain, and June's 82.03 / 87.49 shortfall are already in
`ATLAS §Thresholds` wave by wave. The **confirmatory object is the external leg**: a like-for-like
disclosure comparison of what each provider publishes about its own cells, anchored on the one
common key that now exists — **149 of OpenAI's 165 IWA ids match the June wave's `onet` level-2 node
ids** — with the Index's `onet` L2 `pct` at global and country against the corresponding external
series.

**Supplementary data.** OpenAI Signals bundle (`cdn.openai.com/signals/data-download-csv.zip`), now
verified by the steward: 25 CSVs, 24 months, 125 countries, 165 O*NET IWAs, joining to the June
wave's `onet` L2 node ids on 149 of 165; Microsoft `working-with-ai` `iwa_metrics.csv` (332 IWAs with
completion, scope and feedback columns); the RPS indexes, which publish `number_observations` for
every cell.
...
**Biggest risk.** Design, and the steward has already realised half of it: without the external leg
this post is our own atlas in a table. With it, the risk moves to comparability — three providers
publish different cells of different things, and a disclosure comparison can slide into a
league table of whose data is "better", which is not a research finding and would read as a
competitive claim from a fellowship applicant. If it bites, the post is a standards note; the
pre-registration therefore fixes the confirmatory object as the IWA-level coverage comparison on the
149 matched ids and rules the commentary out in advance.
...
**Contribution** (amended: the steward has run the sweep and the speedup is near-invariant, which
relocates the finding). *If it holds* (the mean speedup is flat across the threshold range, as
11.93× → 11.83× → 11.59× says): the published 1.8pp → ~5pp swing is **not** coming from the speedup
distribution, and the post locates it in the coverage-and-weighting step instead — which is the more
useful answer and the one the footnote obscures. *If it fails* (a steep gradient appears on the
February wave or on a different base): the headline is a function of the inclusion rule and the post
publishes the curve. *If null*: the post publishes the first threshold sweep of the corpus's
most-quoted productivity input, in the report's own unit, with the aggregate discussed only through
its published endpoints.

**Biggest risk.** Data. The published aggregate needs conversation-level speedups and a wage-bill
weighting that no release ships, so the post cannot reproduce 1.8pp and cannot show the aggregate's
threshold curve directly — the steward has now confirmed this rather than left it inferred. The post's
object is therefore the task set and its mean speedup, the aggregate is quoted as published and never
rebuilt, and the title must not promise more than the task set.
...
**Economic Index cut (proposed, amended on the steward's ruling — the entry's premise was wrong).**
All three quantities co-exist in **`release_2026_01_15`**, so no splice is needed: 118 countries
clear 200 conversations, 117 have a working-age population row (so the AUI is rebuildable on the
symmetric, thresholded-only rule, `ATLAS §Conventions`), 116 of those carry IMF 2024 GDP, and all
116 carry `use_case` and `request` L2 at country grain (114 carry `onet_task`). Decomposition of the
AUI–income elasticity into a within-mix component and a mix-composition component, **inside
2026-01-15 at N = 116**, at `request` L1/L2 where cell fill permits and never at `onet_task` L0 below
global (`§Cuts`, Family B fill table). The population and GDP files are static 2024 annuals rather
than wave quantities, so pairing them with the November wave is not a splice.

**Supplementary data.** `gdp_2024_country.csv` and `working_age_pop_2024_country.csv` from
`release_2025_09_15/data/intermediate/`, used as static 2024 annuals.
...
**Biggest risk.** Design, now that the data risk is gone. With everything inside one wave at N = 116,
the binding problem is that the use-case mix and usage intensity are jointly determined — a country
whose users are students has both a different mix and a different per-capita rate — so the
decomposition apportions variance rather than identifying a mechanism. If it bites, the two
components are not separately interpretable; the post reports the decomposition as an accounting
identity, states that neither component is causal (the fourth report says so of its own
correlations), and reports the MDE at 116 countries.
...
|---|---|---|
| LL-32 | Does it matter how a conversation is credited to the occupations that share its task? | **Subsumed by LL-36, and exhausted by the steward's audit.** He marked it FEASIBLE WITH CAVEAT and then answered it: in the O\*NET 20.1 statements a task text has one holder occupation for all but 72 / 91 / 84 tasks — 4.05% / 5.57% / 4.50% of named usage mass — and equal-split, employment-weighted and modal-holder allocation move the SOC-15 share by **≤0.17 pp** and the 22-group ranking by at most one position (August 2025) or none (November, February). His words: "the answer is almost certainly 'the rule barely matters', and the effect size is the finding … Publishable as a bound on a construction choice, not as a correction." He also named LL-32/LL-36 as the pair that could not be two posts. A bound of 0.17 pp is a robustness paragraph, not a post, so the allocation-rule check moves into LL-36 as a pre-registered robustness cut with those numbers attached. Ledger items are rehoused: `L-2025-02-P1-28` to LL-36, `L-2026-03-LMIA-03` already sits in LL-03 and LL-24, `L-2026-06-R6-39` already sits in LL-26. |
| LL-34 | Can a model launch be seen in what people ask AI to do? | **NOT FEASIBLE (steward), deleted on his recommendation.** "Three changes move together, there are no counts, and the two files are not a panel — recommend deletion. The bound does exist and is large: 2,781 common task names (97.35% / 98.11% of each file's mass), total-variation distance **27.24 pp** on the renormalised common set, largest single-task move 1.85 pp, Spearman 0.7475, with 733 v1-only and 584 v2-only tasks. But v1 **is** the December-2024 release, so model, classifier and occupational-relevance filter all differ, and neither file publishes a count, so nothing about a launch is identified. The question survives where the taxonomy is fixed on one side: **LL-39**." (`room/steward-2026-09-16-longlist-feasibility-batch-3-answers.md`.) The entry had written its own deletion condition and the steward met it. Its ledger items stay open: `L-2025-03-R2-17` and `L-2025-03-R2-09` are recorded here as **unreachable on public data** for the long-list's purposes, and `L-2025-03-R2-16` (the v1→v2 comparison never re-run through one pipeline) is partly served by LL-39, which asks the same question at a boundary where one side's taxonomy is fixed. |
