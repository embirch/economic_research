...
|---|---|---|---|
| post1 | LL-07 | Is AI delegated more on low-wage work or on high-wage work? (frozen at Gate 1b) | Gate 1b approved 2026-09-17 (brief 9af7bad + b4ad871; items 14–20, 23 applied 8fbffbd). Replication e9a2291 (all match). Prereg: analyst 47faca0 → referee PASS WITH CHANGES 37868c3 (0 blocking) → revision c9b1b45 → second-read SIGN OFF 0a6513f → committed 066b761; Gate 2a approved 2026-09-17. Analysis 660dad5 (17 confirmatory + 3 exploratory; 4 deviations, both rules run) → referee PASS WITH CHANGES 1b621c0 (raw re-derivation match; claims.md, red-team.md) → revision 698a2f1 → second-read SIGN OFF 09f05ab. **Gate 2b APPROVED 2026-09-17** (room/director-2026-09-17-gate-2b-post1-approved.md); declared O-A, H3 declared 6/8, H1 clause failed; claims.md is the boundary. Stage 3 opened in the same session. Carried to Stage 3: sharpen §2 why-it-matters (human's note); footnote BRIEF §9(4) H4 power ≈0.32 pp as declared (referee-prereg item 3); prereg "could" items 10, 15(c) at results review |
...
|---|---|
| post1 | **In progress (session 2.1, cap $200).** Human's Gate 1b note: the why-it-matters (§2) is to be sharpened at the write-up stage by the editor with the lead — lead's one-page room note → editor. Gate 2b reader's note: the write-up must make the reader understand what the results mean and why they matter, not just report them. Branch `post1-draft`; never push the page to main; Gate 3 = branch, commit, POST.md and page paths; the human merges. Footnote against BRIEF §9(4): declared-H4 has 80% power only up to ≈0.32 pp; the ≈0.5 pp is step (4)'s clause (posts/post1/notes/referee-prereg.md item 3). |
...
| 2.1 post1 Stage 2 (Gate 2a → Gate 2b) | 8 threads to Gate 2b: lead, steward, analyst (prereg, two turns), referee (prereg), second-read (prereg), analyst (results, two turns), referee (results), second-read (results) | zero billing stops, zero send-backs, zero tree conflicts; Gate 2a and 2b approved same day; Stage 3 continues in-session under a $200 cap |
