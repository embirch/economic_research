...
- **Official API extract, 21 September 2026 (later the same day):** the Eurostat SDMX endpoint served both tables (`data/fetch/eurostat_isoc_ai_iaiu.py`; `isoc_ai_iaiu.tsv` sha256 `7f668f7b…`, `isoc_ai_iaiuxr.tsv` `8901c5c9…`). Cell-by-cell comparison with the EIGE mirror: 37,885 common cells, **0 value differences, 0 flag differences**; the official file adds 1,121 cells for the euro-area aggregate `EA`, absent from the mirror. The official TSV is the extract of record from here; its key order is `freq,ind_type,indic_is,unit,geo`, values carry the flag after a space, and missing is `:`.
