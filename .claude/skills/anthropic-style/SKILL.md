...
## Proposed changes to team/templates/POST.md

Consistent with the template as written. Three additions are proposed to the director in `room/editor-2026-09-16-criteria-proposal.md` and are not applied here: a **Recommendations to Anthropic** heading of its own (the template lists recommendations in the criteria but not in the section list); **Reproduction** and **Assistance disclosure** as named headings rather than clauses inside Methodology; and an explicit **What was set in advance** heading, which the criteria require and the template folds into Methodology. Until the director rules, follow the section order in `team/templates/POST.md` and put the three inside Methodology in that order.
