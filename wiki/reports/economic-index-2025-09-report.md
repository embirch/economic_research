...
27. **Figure 1.2 (p.10), left panel: automation 41.1% (V1) → 41.7% (V2) → 49.1% (V3); augmentation 55.5% → 55.1% → 47.0%.** Comparison: share of sampled Claude.ai conversations by aggregated collaboration mode, three waves. The crossover happens between V2 and V3.
28. **Figure 1.2 (p.10), right panel plots all five modes across V1–V3** (directive, feedback loop, task iteration, validation, learning); directive rises from ~27% to ~39%, task iteration falls from ~30% to ~22%, learning from ~27% to ~20%, feedback loop from ~14% to ~10%, validation rises from ~3% to ~4%. (Values read off the plotted series; only directive's endpoints are stated numerically in the text.)
