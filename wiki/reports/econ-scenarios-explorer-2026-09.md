...
| Explorer model bundle | The model's calibration, scenario presets and quiz wording, shipped in the page's JavaScript chunk | **`(explorer bundle)`** — ruled a published asset by `room/director-2026-09-16-corpus-audit-answer.md` D1 |

`(figure label)` follows `room/director-2026-09-16-figure-values-ruling.md`. `(chart data asset)` follows `room/director-2026-09-16-chart-asset-ruling.md`; the only difference from the precedent case is that this page's chart data is embedded in the page payload rather than served as a separate `.json` file, which if anything makes it *more* clearly part of the published page. `(explorer bundle)` goes beyond both of those rulings and was flagged, not assumed; it is now covered by `room/director-2026-09-16-corpus-audit-answer.md` D1, which holds that the bundle served unauthenticated with the page is a published asset and that no value marked this way enters a post unless the steward reproduces it from a release or the source states it in prose — see `Verification`. **No number in this file was read off a chart image**, and three of the page's four findings are illustrated by charts that render client-side with no values in the source at all (see `Verification`).
...
*Comparison:* the 2026 stock against the 2030 stock, with the flow between them shown as a middle column; all as percentages of **all** workers. The key names three states: "Knowledge workers", "All other workers", "Displaced". Note that the figure's 2026 column, 62.2% / 37.8%, is **close to but not identical with** the model's base-period employment shares of 0.6235 / 0.3765, i.e. 62.35% / 37.65% (see `Data and methods`). Which of the two readings is right — the calibration input rounded for display, or the model's simulated mid-2026 state, AI being active from the 2024 base — is **not established** by the page or the bundle. *(An earlier version of this note concluded "this figure's left column is a calibration input, not a model output"; that inference is withdrawn per `room/referee-2026-09-16-corpus-audit.md` defect L3.)*
...
- **Two groups, fixed at the start.** *Cognitive* (SOC 11–29, 41, 43) and the rest. Employment shares in the base period 0.6235 / 0.3765, from CPS 2025 annual averages **(explorer bundle)** — close to, but not the same number as, the 62.2% / 37.8% left column of the reallocation figure (62.35% against 62.2%). The two are **not** asserted here to be the same quantity; see the note to `Claims` 14.
...
- **Chart data asset, per `room/director-2026-09-16-chart-asset-ruling.md`.** **Twelve places** in this file carry a value or label from the survey payload at the point of use, each marked `(chart data asset)`; the marker is named four more times in `## Source` and in this section while the convention is explained. The ruling's condition — values in a published chart data asset, JSON served by the page to draw its charts — is met; the only difference from the precedent case is that this payload is embedded in the page rather than served as a separate file, which strengthens rather than weakens the case. Nothing here was read off an image. **The dependency matters:** claims 34, 36 and 37 and `What it did not test` 13 and 15 rest on it, and item 37 — the five different per-question denominators — is the entry's single most consequential finding about the survey and exists nowhere else.
- **Explorer bundle — recorded under the director's ruling.** **Twenty places** in this file carry a value, label or quoted string from the page's JavaScript model bundle at the point of use, each marked `(explorer bundle)`; the marker is named six more times in `## Source` and in this section while the convention and the ruling are explained. This goes beyond both earlier rulings: the chart-asset ruling covers data served to draw charts, and this is *code and its constants*. The argument for recording it: the bundle is the explorer's source of record for what the explorer computes, the `Data and methods` section could not otherwise say what the tool does, and the material is served unauthenticated from anthropic.com as part of the published page. The argument against: it carries internal build and provenance annotations (a source repository path, commit hashes, an internal decision-log identifier scheme, input-file checksums and unfinished placeholder copy) that were plainly not written for readers, which is evidence that the *annotations*, at least, are not a publication. **Ruled on, 2026-09-16** (`room/director-2026-09-16-corpus-audit-answer.md` D1, answering `room/referee-2026-09-16-corpus-audit.md`): the bundle served unauthenticated with the published page is a published asset, like a chart's data JSON; constants, anchors and quiz wording read from it may be recorded here, each marked `(explorer bundle)` with the chunk name, exactly as this file does — while internal build annotations, variable names and anything not user-facing stay out, and **no `(explorer bundle)` value enters a post unless the steward reproduces it from a release or the source states it in prose.** Had the ruling gone the other way, the marked passages would have come out: `Definitions` would lose the full question wording and the group definitions, `Data and methods` the preset table, the calibration table and the Claude-traffic anchor, and `What it did not test` 4, 6, 7 and 9 their support. Internal identifiers, hashes and repository paths are recorded nowhere in this file by intent; only their existence is noted, because it bore on whether the bundle may be cited at all.
...
- **Quotation check.** Every block quotation in this file was checked string-for-string against the fetched sources by script, after normalising whitespace, curly quotes, dashes and markdown emphasis. **63 block quotations, all matched:** 51 against the page's own text (rendered fetch and raw HTML agreeing), 11 against the explorer bundle or the survey payload — each of those 11 marked at the point of use — and one, the "Tasks leave the bundle…" passage, matched against the raw HTML with markup removed, because the page splits its parentheticals across inline elements and the text extractor inserts spaces the rendered fetch does not. Two strings that are **templates rather than fixed text** (the capability slider's "today" and "beyond the paper" notes, which interpolate a computed integer) are deliberately **not** presented as quotations; their computed values are given as the wiki author's evaluation. Source oddities reproduced deliberately: "share of respondents at each answers" (survey chart title) and the page's own inconsistency between "substantial scenario" and "substantial change" scenario. **Inline quotations in `## Claims` were checked too** (added 2026-09-16 after `room/referee-2026-09-16-corpus-audit.md` defect L2(c), which noted the earlier wording asserted a check only of block quotations): the page was re-fetched by `curl` on 2026-09-16 (HTTP 200, 383,504 bytes) and all **89 inline quotations of eight characters or more in `## Claims`** were matched by script against the normalised page text and the page's script payload. **76 matched directly.** The other 13 are accounted for and none is a wording error: nine are page prose or SVG text that the markup splits — React inserts `<!-- -->` separators around interpolated numbers ("15% a year", "60¢ … 40¢", "n = 10,980") and wraps single words in `<span>` ("the modest scenario", "the substantial scenario", "the extreme scenario"), and one is the reallocation figure's `aria-label`, which lives in an attribute; three are the belief-swarm tick-label sets, which this file joins with " / " for readability and which appear in the source as separate `<span>` elements; and one, at `Claims` 14, is this file's own withdrawn inference, quoted as its own words and not as the source's.
- **Marker counts, recounted 2026-09-16** (the earlier line said 24 / 13 / 10; the last two were wrong, `room/referee-2026-09-16-corpus-audit.md` defect L2(b)). **At points of use — that is, everywhere outside `## Source` and this `## Verification` section, where each marker is also named while explaining it — the counts are: `(explorer bundle)` 20, `(chart data asset)` 12, `(figure label)` 7**; the seven are the reallocation figure's values in `Claims` 14. Counting every occurrence of the literal string, including the layer table, the ruling sentences and these bullets, gives 26, 16 and 11. No use of `(alt text)`, for the reason given above.
