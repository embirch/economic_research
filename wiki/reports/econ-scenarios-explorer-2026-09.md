...
- **Marker counts.** 24 uses of `(explorer bundle)`, 13 of `(chart data asset)`, 10 of `(figure label)` (seven of them the reallocation figure's values in `Claims` 14, the rest in the layer table and here). No use of `(alt text)`, for the reason given above.
