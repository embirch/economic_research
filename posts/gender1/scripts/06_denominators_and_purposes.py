...
h_work = {"usable_set": work_set, "N": N_w, "raw_favour": fav, "raw_majority": fav > N_w / 2, "threshold_19": fav >= 19,
          ...
          "verdict": ("supported" if fav > N_w / 2 else "against"), "reading": ("not distinguishable from an even split" if fav < 19 and fav > N_w - 19 else ("beyond the 0.026 chance threshold" if fav >= 19 or fav <= N_w - 19 else "")),
...
for ind, name in [("I_IUAIPR", "private"), ("I_IUAIWP", "work"), ("I_IUAIFE", "education")]:
    ...
    purposes[name] = {"participation": {"N": len(part), "positive": sum(v > 0 for v in part.values()), "negative": sum(v < 0 for v in part.values()), "median_gap": sorted(part.values())[len(part) // 2], "values": part,
                                        "eu27": overall["EU27_2020"].get(f"{ind}|PC_IND")},
                      "among_users": {"N": len(among), "positive": sum(v > 0 for v in among.values()), "negative": sum(v < 0 for v in among.values()), "median_gap": sorted(among.values())[len(among) // 2], "values": among,
...
band_medians = {b: sorted(internet[g]["bands"][b]["composition_share"] for g in comp_set)[len(comp_set) // 2] for b in BANDS}
...
sets = {"eu27": {"N": 27, "median": sorted(eu_gaps.values())[13], "reversed": sorted(g for g, v in eu_gaps.items() if v < 0)},
        "extension": {"N": len(ext_gaps), "median": sorted(ext_gaps.values())[len(ext_gaps) // 2], "reversed": sorted(g for g, v in ext_gaps.items() if v < 0)},
        "all35": {"N": 27 + len(ext_gaps), "median": sorted(list(eu_gaps.values()) + list(ext_gaps.values()))[(27 + len(ext_gaps)) // 2]}}
...
write_json("denominators_purposes.json", {"h_work": h_work, "purposes": purposes, "internet": internet, "internet_composition_band_medians": band_medians,
                                          "internet_set_N": len(comp_set), "flagged_appendix": flagged_appendix, "sets": sets})
