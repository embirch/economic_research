...
for i in range(len(order2)):
    for j in range(6): ax.text(j, i, f"{M[i, j]:+.0f}", ha="center", va="center", fontsize=6.5, color="black" if abs(M[i, j]) < 8 else "white")
...
for k, (name, title) in enumerate([("private", "Private use"), ("work", "Work use"), ("education", "Formal education")]):
    ...
    gs = sorted(part, key=lambda g: part[g]); yy = np.arange(len(gs))
    ax[k].scatter([part[g] for g in gs], yy, color="#4a6fa5", s=18, label="of all individuals")
    ax[k].scatter([among.get(g, np.nan) for g in gs], yy, color="#c8553d", s=18, marker="s", label="of AI users")
    ax[k].axvline(0, color="black", lw=.8); ax[k].set_title(title, fontsize=10); ax[k].set_yticks(yy); ax[k].set_yticklabels([NAME[g] for g in gs], fontsize=7)
...
ax.set_xlabel("crude gap, men minus women, points"); ax.set_ylabel("gap under the EU27 age structure, points"); ax.set_title(f"Age standardisation moves {len(S['h_composition']['changed'])} of {len(comp)} countries across a tercile cut (red)", fontsize=9)
...
ax.axhline(0, color="black", lw=.8); ax.set_xticks(range(6)); ax.set_xticklabels(["16–24", "25–34", "35–44", "45–54", "55–64", "65–74"]); ax.set_ylabel("gap among all individuals minus gap among internet users, points"); ax.legend(fontsize=8)
...
res = {"post": "gender1", "question": "Where is the gender gap in generative AI use widest, and how much of that is measurement?", "generated": __import__("time").strftime("%Y-%m-%dT%H:%M:%SZ", __import__("time").gmtime()),
       ...
       "classes": {"counts": S["class_counts"], "counts_distinguishable": S["class_counts_distinguishable"], "large_gap": cls_list("large-gap"), "small_gap": cls_list("small-gap"), "reversed_on_both_denominators": sorted(g for g, v in cls.items() if v["reversed_both_denominators"]),
                   "reversed_on_pc_ind": sorted(g for g in EU27 if ov[g]["I_IUAI|PC_IND"]["gap"] < 0), "not_classifiable": cls_list("not classifiable"), "cuts": S["cuts"], "tercile_changes": {"gap_vs_ratio": G["terciles"]["changes_gap_vs_ratio"], "crude_vs_standardised": S["h_composition"]["tercile_changes"], "ratio_vs_standardised": S["changes_ratio_vs_std"]}},
       ...
       "internet_composition": {"band_medians": D["internet_composition_band_medians"], "N": D["internet_set_N"], "eu27": D["internet"]["EU27_2020"]},
...
figs = {"fig1": {"file": "outputs/figures/fig1_rates_and_gaps.png", "script": "scripts/08_results_and_figures.py", "caption": "**In every EU country but five, men were more likely than women to have used generative AI in early 2025; the gap runs from about −2 to +9 points and its class is stable across measures for only 13 of 27.** Plotted, left: the share of individuals aged 16–74 who used generative AI in the three months before the interview, by sex, EU member states, 2025 ICT survey (dotted lines: EU27 men and women). Right: men minus women in percentage points; bar colour is the persistent class under the registered rule (blue large-gap, red small-gap, grey not distinguishable, light grey not classifiable); the black line is the simple-random-sampling 95% bound from the national net sample, a lower bound; ● marks a class that survives that bound. Source: Eurostat isoc_ai_iaiu, PC_IND; N = 27."},
        ...
        "fig4": {"file": "outputs/figures/fig4_crude_vs_standardised.png", "script": "scripts/08_results_and_figures.py", "caption": "**Applying the EU27 age structure to every country lowers the gap in 25 of 26 and moves three across a tercile cut.** Plotted: each country's crude gap (x) against its gap after direct age standardisation with the EU27 2025 sex-pooled population weights (y); the dotted line is equality; red marks a country whose tercile differs between the two. Standardisation is defined only where all twelve band cells are usable (26 of 27; Ireland excluded). Source: Eurostat isoc_ai_iaiu and demo_pjan."},
        "fig5": {"file": "outputs/figures/fig5_internet_composition.png", "script": "scripts/08_results_and_figures.py", "caption": "**Little of the gender gap is an internet-use gap: measuring among internet users instead of all individuals changes the gap by under a point at the median in every band, and by more than a point in ten countries only at 65–74.** Plotted: the gap among all individuals minus the gap among recent internet users, by age band, one grey line per EU member with all six bands usable (26), the red line the median across them, the blue line the EU27 aggregate. Source: Eurostat isoc_ai_iaiu, PC_IND and PC_IND_IU3."}}
