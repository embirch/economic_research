...
for i in range(len(order2)):
    for j in range(6): ax.text(j, i, ("0" if abs(M[i, j]) < 0.5 else f"{M[i, j]:+.0f}"), ha="center", va="center", fontsize=6.5, color="black" if abs(M[i, j]) < 8 else "white")
...
res = {"post": "gender1", "question": "Where is the gender gap in generative AI use widest, and how much of that is measurement?", "generated": __import__("time").strftime("%Y-%m-%dT%H:%M:%SZ", __import__("time").gmtime()),
       ...
       "internet_composition": {"band_medians": D["internet_composition_band_medians"], "N": D["internet_set_N"], "eu27": D["internet"]["EU27_2020"], "eu27_decomposition": D["internet_decomposition_eu27"],
                                "reading": "a difference of gaps on two denominators; where internet use is below 100% its sign is a rescaling of the internet-user gap, not a composition that favours women; the sex difference in implied internet use is within a point in every band at EU27 level"},
